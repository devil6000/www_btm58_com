...
if($op=='display'){
	...
	if($_GPC['export']==1){
		...
		$this->exportexcel($arr, array('提现单号', '粉丝昵称', '手机号码','提现方式','提现帐号','提现帐号人姓名', '提现类型', '申请佣金', '申请时间', '处理方式', '处理时间','状态','商户订单号','微信订单号','管理员备注'), $filename);
		exit();
	}
	...
	if(checksubmit()){
		...
		$data = array();
		if($type==1){
			if($amount<0 && $user['nopay_commission'] < abs($amount)){
				message("用户未提现佣金不足，当前未提现佣金：".$user['nopay_commission']."元", "", "error");
			}
		...
		try {
			...
			load()->func('logging');
			logging_run('管理员后台操作分销商佣金失败(uniacid:'.$uniacid.')，原因：'.$e->getMessage(), 'trace', 'fylessonv2_finance');
			pdo_rollback(); 
		}
