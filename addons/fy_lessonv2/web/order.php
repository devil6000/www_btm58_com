...
if($operation == 'display') {
	...
	if($_GPC['export']==1){
		...
		foreach ($outputlist as $key => $value) {
			...
			$arr[$i]['spec_name']		= $value['spec_name'];
			if($value['lesson_type']==0){
				$arr[$i]['lesson_type_name'] = "普通课程";
			}elseif($value['lesson_type']==1){
				$arr[$i]['lesson_type_name'] = "报名课程";
			}
			...
			if($value['lesson_type']==1){
				$arr[$i]['verify_name'] = $value['is_verify']==1 ? '未核销' : '已核销';
			}else{
				$arr[$i]['verify_name'] ='';
			}
		...
		$this->exportexcel($arr, array('订单编号','昵称','姓名','手机号码','规格名称','课程类型','课程名称','课程价格','获赠积分','付款方式','一级佣金','二级佣金','三级佣金','订单状态','下单时间','核销状态','报名课程信息'), $filename);
		exit();
	}
