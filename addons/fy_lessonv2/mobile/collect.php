...
if($ctype==1){
	...
	$lessonlist = pdo_fetchall("SELECT a.id,a.bookname,a.images,a.price,a.buynum,a.virtual_buynum,a.section_status FROM " . tablename($this->table_lesson_parent) . " a LEFT JOIN " .tablename($this->table_lesson_collect). " b ON a.id=b.outid WHERE {$condition} ORDER BY b.addtime DESC  LIMIT " . ($pindex-1) * $psize . ',' . $psize, $params);
