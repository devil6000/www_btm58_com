...
if($op == 'display') {
	...
	$video_player = $typeStatus->videoPlayerType();
    ...
    if (checksubmit('submit')) {
		...
		/* 阿里云点播 */
        $aliyuntype = array(
            'region_id'		=> trim($_GPC['aliyun']['region_id']),
            'access_key_id'	=> trim($_GPC['aliyun']['access_key_id']),
            'access_key_secret'  => trim($_GPC['aliyun']['access_key_secret']),
        );
        $data['aliyun'] = serialize($aliyuntype);
