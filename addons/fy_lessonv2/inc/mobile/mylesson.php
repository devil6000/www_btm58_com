...
if(!preg_match('/&status=/',$_W['siteurl']) && $op!='cancle'){
	header("Location:".$_W['siteurl'].'&status=');
}
...
if($is_verify=='0'){
	$condition .= " AND a.is_verify=:is_verify AND a.lesson_type=:lesson_type ";
	$params[':is_verify'] = 0;
...
if($op=='display'){
	...
	if($is_delete){
		...
		if(pdo_update($this->table_order, array('is_delete'=>'1'), array('id'=>$orderid))){
			message("删除成功", $this->createMobileUrl('mylesson').'&status=', "success");
		}else{
			message("删除失败", "", "error");
		}
	}
	...
	if(pdo_update($this->table_order, array('status'=>'-1'), array('id'=>$orderid))){
		...
		message("取消成功", $this->createMobileUrl('mylesson').'&status=', "success");
	}else{
		message("取消失败", "", "error");
	}
