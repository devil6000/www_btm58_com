...
if($op=='display'){
	...
	$list = pdo_fetchall("SELECT * FROM " . tablename($this->table_article) . " WHERE uniacid=:uniacid AND isshow=:isshow ORDER BY displayorder DESC, id DESC  LIMIT " . ($pindex-1) * $psize . ',' . $psize, array(':uniacid'=>$uniacid, ':isshow'=>1));
