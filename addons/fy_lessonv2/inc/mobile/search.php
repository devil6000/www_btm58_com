...
if((!empty($login_visit) && in_array('search', $login_visit)) || $userAgent){
	checkauth();
}
...
if ($op == 'display') {
	...
	$hot_category = pdo_fetchall("SELECT * FROM " . tablename($this -> table_category) . " WHERE uniacid=:uniacid AND is_hot=:is_hot ORDER BY displayorder DESC", array(':uniacid' => $uniacid, ':is_hot' => 1));
