...
if($op=='display'){
	...
	if($order['status']==2){
		message("该订单已评价！", $this->createMobileUrl('mylesson')."&status=", "warning");
	}
	...
	if(checksubmit('submit')){
		...
		if($result){
			...
			message($evaluate_word, $this->createMobileUrl('mylesson')."&status=", "success");
		}else{
			message("评价失败！", $this->createMobileUrl('mylesson')."&status=", "error");
		}
	}
