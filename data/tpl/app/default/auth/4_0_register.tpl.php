...
<script>
		...
		$(document).on('click', '.send-code', function(){
			...
			var timer = setInterval(function(){
				...
				if(downcount <= 0){
					clearInterval(timer);
					downcount = 60;
					$('.js-timer').html('<a class="send-code" uniacid="<?php  echo $_W['uniacid'];?>">重新获取验证码</a>');
				}else{
					$('.js-timer').html(downcount + "秒后重新获取");
				}
			}, 1000);
	...
	<div class="mui-content mc-login">
		...
		<div class="mui-input-group" id="register-basic">
			<div class="mui-input-row">
				...
				<input name="username" type="text" placeholder="<?php  if($item == 'random') { ?>请填写手机号/邮箱<?php  } else if($item == 'email') { ?>请填写邮箱<?php  } else { ?>请填写手机号<?php  } ?>"/>
			</div>
			<div class="mui-input-row">
	...
	<?php  if($rtype == 'mobile' || ($item == 'mobile' && $ltype == 'code')) { ?>
	...
	<div class="mui-content-padded">
		<div class="mui-text-center mui-mt15">
			<a href="<?php  echo url('auth/login', array('forward' => $_GPC['forward']));?>">立即登录</a>
			<?php  if($rtype == 'email') { ?>
			<?php  if(($item == 'mobile' && ($ltype =='hybird')) || ($item == 'random' && ($ltype !== 'password'))) { ?>
			<span class="mui-ml5 mui-mr5 mui-text-muted">|</span>
			<a href="<?php  echo url('auth/register', array('forward' => $_GPC['forward'], 'type' => 'mobile'));?>#wechat_redirect">无密码注册
			</a>
			<?php  } ?>
			<?php  } else { ?>
			<span class="mui-ml5 mui-mr5 mui-text-muted">|</span>
			<a href="<?php  echo url('auth/register', array('forward' => $_GPC['forward'], 'type' => 'email'));?>#wechat_redirect">邮箱注册
			</a>
			<?php  } ?>

			<span class="mui-ml5 mui-mr5 mui-text-muted">|</span>
			<a href="<?php  echo url('auth/forget', array('forward' => $_GPC['forward']));?>">找回密码</a>
		</div>
	</div>
</div>
