	...
	<form action="" method="post" class="form-horizontal form" enctype="multipart/form-data">
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="form-group">
					<label class="col-xs-12 col-sm-3 col-md-2 control-label">个人中心菜单</label>
					<div class="col-sm-9">
						<label class="radio-inline" style="padding-left:10px;">
							<input type="checkbox" name="self_item[]" value="vip" <?php  if(in_array('vip',$self_item)) { ?>checked<?php  } ?>> VIP服务
						</label>
						<label class="radio-inline">
							<input type="checkbox" name="self_item[]" value="history" <?php  if(in_array('history',$self_item)) { ?>checked<?php  } ?>> 我的足迹
						</label>
						<label class="radio-inline">
							<input type="checkbox" name="self_item[]" value="lesson" <?php  if(in_array('lesson',$self_item)) { ?>checked<?php  } ?>> 收藏课程
						</label>
						<label class="radio-inline">
							<input type="checkbox" name="self_item[]" value="teacher" <?php  if(in_array('teacher',$self_item)) { ?>checked<?php  } ?>> 收藏讲师
						</label>
						<label class="radio-inline">
							<input type="checkbox" name="self_item[]" value="teachercenter" <?php  if(in_array('teachercenter',$self_item)) { ?>checked<?php  } ?>> 讲师中心
						</label>
						<label class="radio-inline">
							<input type="checkbox" name="self_item[]" value="coupon" <?php  if(in_array('coupon',$self_item)) { ?>checked<?php  } ?>> 优惠券
						</label>
						<label class="radio-inline" style="padding-left:0;">
							<input type="checkbox" name="self_item[]" value="mobile" <?php  if(in_array('mobile',$self_item)) { ?>checked<?php  } ?>> 绑定/修改手机
						</label>
						<label class="radio-inline" style="padding-left:0;">
							<input type="checkbox" name="self_item[]" value="setting" <?php  if(in_array('setting',$self_item)) { ?>checked<?php  } ?>> 设置按钮
						</label>
						<span class="help-block">勾选的自定义菜单会显示在手机端的个人中心里；已申请通过审核的讲师继续显示讲师中心</span>
					</div>
				</div>
